...
// Admin: Create a gift card without payment
export const adminCreateGiftCard = async (req, res) => {
  try {
    // Check if user is admin
    if (!req.user.isAdmin) {
      return res.status(403).json({
    ...
    const { amount, recipientName, recipientEmail, currency, message } = req.body;
    
    // Validate input
    if (!amount || !recipientName || !recipientEmail || !currency) {
      return res.status(400).json({
        success: false,
        message: 'Missing required fields'
      });
    ...
    const giftCard = new GiftCard({
      ...
      message: message || '', // Store message if provided
      paymentIntentId: 'admin-created', // No payment for admin-created gift cards
      expiryDate
    });
    ...
    console.error('Error creating gift card by admin:', error);
    return res.status(500).json({
      success: false,
      message: 'An error occurred while creating gift card',
      error: error.message
